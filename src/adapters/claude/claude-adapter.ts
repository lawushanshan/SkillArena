 ...
 * `--output-format json` for structured JSON output. Trace capabilities are
...
export async function runClaudeExec(options: ClaudeExecOptions): Promise<AgentExecResult> {
  ...
  const args = [...claudeCommandArgs, "--print", "--output-format", "json", options.prompt];
  let stdout = "";
  let stderr = "";
  let timedOut = false;
...
/**
 * Parse Claude Code CLI output into SkillArena's normalized trace model.
 *
 * TODO: Claude Code's `--output-format json` produces structured output but
 * the schema for tool use, file changes, and command execution events needs
 * to be mapped to SkillArena's normalized event model. This initial
 * implementation returns an empty event list.
 */
